...
Aprobado por el dueño el 25/07/2026 como **modelo objetivo**: piloto automático con topes. ⚠️ **Estado real:** los scripts están **creados pero NO instalados** en la cuenta (ver columna Estado), así que hoy **no corre ningún autopilot ni hay tope de gasto activo**. No comunicar "tope duro vigente" sin captura del trigger en la UI de Google Ads. Detalle y política: `06-google-ads-scripts.md`.

| Script | Frecuencia | Hace solo | Estado |
|--------|------------|-----------|--------|
| `05-autopilot.js` | Semanal (lun 07:00) | Negativas de lista negra, negativas exactas de términos que gastan sin convertir, pausa de keywords sangrantes. Máx. 25 negativas + 5 pausas por corrida; marca protegida; email de cada cambio | ⏳ **Pendiente de instalar** (sesión de Chrome) |
| `06-guardian-presupuesto.js` | **Diario** (06:00) | Red secundaria del tope **ARS 300.000/mes**: avisa al 80%, pausa todo al ~95% (margen por latencia), reactiva al cambiar el mes. Heartbeat + kill-switch. El freno duro por día es el **presupuesto nativo ≈ tope/30** (en la UI) | ⏳ **Pendiente de instalar** |
| `02-reporte-semanal.js` | Semanal | Solo lee: reporte de rendimiento por email | ⏳ Pendiente de instalar |
| `03-ngram-terminos.js` | Semanal | Solo lee: análisis n-gram para negativas | ⏳ Pendiente de instalar |
| `04-chequeo-urls.js` | Diario en migración, luego semanal | Solo lee: alerta URLs finales rotas | ⏳ Pendiente de instalar |
...
1. **Sesión de Chrome** (dueño mira, Claude maneja): instalar los 5 scripts en Google Ads, auditar la cuenta abandonada, armar la campaña de relanzamiento (`05-google-ads-operacion.md`, Fase 1 y 2).
